...
| Función | Línea | Qué hace | Depende de | La usan |
|---------|-------|----------|-----------|---------|
| `adProcessFile(input)` | 3567 | Lee un archivo Excel con SheetJS, detecta columnas por nombre, construye `AD_RECORDS[]` según la cadena seleccionada (`xtra`, `machetazo`, `ricamar`, `ribasmith`, `rey`). Para XTRA asigna `vendedor` via `TIENDAS_VENDEDORES[tienda]`. Para Machetazo usa `buscarVendedorMachetazo()` + `MACHETAZO_NOMBRE_MAP`. | `XLSX`, `TIENDAS_VENDEDORES`, `buscarVendedorMachetazo()`, `MACHETAZO_NOMBRE_MAP`, `AD_RECORDS` | Input file "Seleccionar Excel" |
